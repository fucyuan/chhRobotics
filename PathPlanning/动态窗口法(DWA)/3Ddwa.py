...
class Robot:
    def __init__(self, x, y, z, vx, vy, vz, v_max,v_min, a_max,a_min, dt, alpha=0.5, beta=10.0, gamma=0.2):
        """
        初始化机器人状态和参数
...
def evaluate_trajectory(trajectory, goal, obstacles, robot, safe_distance):
    ...
    average_speed = total_distance / (len(trajectory) * robot.dt)
    ...
    # 评价函数改进：
    # - 使用距离指数函数处理目标距离
    # - 对障碍物安全距离进行缓冲处理，低于安全距离时显著降低得分
    obstacle_score = np.exp(-min_obstacle_distance / safe_distance) if min_obstacle_distance < safe_distance else 1.0
    goal_score = 1 / (goal_distance + 1e-6)  # 防止分母为零
    speed_score = average_speed / robot.v_max  # 归一化速度得分
...
def dynamic_window_approach(robot, goal, obstacles, safe_distance):
    ...
    # Sample velocities
    for vx in np.linspace(-robot.v_max, robot.v_max, 10):
        for vy in np.linspace(-robot.v_max, robot.v_max, 10):
            for vz in np.linspace(-robot.v_max, robot.v_max, 10):
...
robot = Robot(x=1, y=1, z=10, vx=0, vy=0, vz=0, v_max=1.0, a_max=0.5, dt=0.1)

# Create the moving goal
goal = MovingGoal(x=18, y=18, z=18, bounds=bounds, v_max=5, direction_change_interval=50)
...
dynamic_obstacles = [
    DynamicObstacle(x=3, y=3, z=3, vx=0.2, vy=0.2, vz=0.1, bounds=bounds, v_max=5),
    DynamicObstacle(x=8, y=8, z=8, vx=-0.1, vy=0.2, vz=0.15, bounds=bounds, v_max=5)
]
